...
if $lossless; then
  quality_str="-x265-params lossless=1"
else
  quality_str="-crf:v $quality"
fi

# Run ffmpeg
ffmpeg -i "$input_file" -c:v libx265 $scale_filter -preset:v slow $quality_str \
